"""Bounded public HTML and linked JSON discovery on one authorized municipality host.

Adapted from Patrick's ``publicai/src/publicai/crawler.py`` (GET-only, robots.txt
respected, DNS pinned to public addresses, byte/time/request budgets, no cookies,
no query strings). The one change: the authorized host is a constructor argument
instead of the hard-coded ``www.ausserberg.ch``, so the same crawler serves every
Municipality in ``data/municipalities.yml``. It is still exactly one host per
crawler instance and still not configurable from page content.
"""
...
USER_AGENT = "MMP-Builder/0.1 (+https://github.com/dominik-scherrer/public-ai-challenge)"
...
_SERVICE_WORDS = re.compile(
    ...
    r"umzug|anmeld|abmeld|mangel|schaden|meldung|offnung|oeffnung|schul|kinderbetreuung|sozial|"
    r"bescheinig|hund|steuer|dienstleistung",
    re.I,
)
...
# i-web / OneGov style service catalogues ("/dienstleistungen/6348", "/aemter/2551")
_SERVICE_PATH = re.compile(r"/(dienstleistungen|aemter|abteilungen|online-?schalter|lebenslagen|abfall)", re.I)
_MAX_JSON_DEPTH = 16
_MAX_JSON_NODES = 10000
_MAX_HTML_DEPTH = 128


def validate_handoff_url(value: str) -> None:
    """Syntax check for an observed link (not fetched): public http(s), no credentials."""
    url = urlsplit(value)
    if (
...
class PublicNetworkBackend(httpcore.AsyncNetworkBackend):
    ...
    async def connect_tcp(
        ...
        if host != self._host or port not in {80, 443}:
...
class _PinnedTransport(httpx.AsyncBaseTransport):
    ...
    def __init__(self, resolver: Resolver, concurrency: int, host: str) -> None:
        self.pool = httpcore.AsyncConnectionPool(
            ssl_context=ssl.create_default_context(),
            max_connections=concurrency,
            max_keepalive_connections=0,
            network_backend=PublicNetworkBackend(host, resolver),
        )
...
def _link(
    ...
    if document_kind == "pdf" or suffix.endswith(".pdf") or label.casefold().endswith(".pdf"):
        ...
        kind = "calendar"
    elif parts.hostname != (host or urlsplit(base).hostname):
        kind = "external"
...
class SafeCrawler:
    ...
    def __init__(
        ...
        *,
        ...
        self._client = httpx.AsyncClient(
            transport=transport or _PinnedTransport(self._resolver, self.settings.concurrency, self.host),
            trust_env=False,
    ...
    def _record(self, url: str, error: CrawlError) -> None:
        try:
            safe_url = validate_url(url, self.host)
        except CrawlError:
            ...
            try:
                async with asyncio.timeout(timeout):
                    await _public_addresses(
                        self._resolver, self.host, 443 if parts.scheme == "https" else 80
                    )
                    self._remaining()
    ...
    async def _read(
        ...
        *,
        ...
        current = validate_url(url, self.host)
        for redirects in range(self.settings.max_redirects + 1):
            ...
            if status in {301, 302, 303, 307, 308}:
                ...
                current = validate_url(urljoin(current, headers["location"]), self.host)
                continue
    ...
    async def fetch(self, url: str) -> FetchedPage:
        ...
        try:
            normalized = validate_url(url, self.host)
            ...
            try:
                ...
                for element in root.iter():
                    if element.tag.rsplit("}", 1)[-1] == "loc" and element.text:
                        candidate = validate_url(element.text.strip(), self.host)
    ...
    async def crawl(self, start_url: str, *, max_pages: int = 40) -> list[FetchedPage]:
        """Seed, then breadth-first over same-host links that look like Services.

        Added for MMP (Patrick's flow let an agent choose follow-up pages). The
        order is deterministic and bounded by ``max_pages`` and the request budget.
        """
        await self.seed(start_url)
        queue: list[str] = list(self.discovered_urls)
        for page in list(self.pages.values()):
            queue.extend(link.url for link in page.links if link.kind == "html")
        seen: set[str] = set(self.pages)
        while queue and len({p.url for p in self.pages.values()}) < max_pages:
            url = queue.pop(0)
            try:
                url = validate_url(url, self.host)
            except CrawlError:
                continue
            if url in seen:
                continue
            seen.add(url)
            if not _SERVICE_WORDS.search(url) and not _SERVICE_PATH.search(url):
                continue
            try:
                page = await self.fetch(url)
            except CrawlError as error:
                if error.reason == "crawl_limit":
                    break
                continue
            queue.extend(link.url for link in page.links if link.kind == "html")
        return list({page.url: page for page in self.pages.values()}.values())
