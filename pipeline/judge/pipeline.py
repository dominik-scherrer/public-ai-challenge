...
Usage:
    uv run python -m pipeline.judge.pipeline \\
        pipeline/handoff/delivery-2026-09-24/ausserberg/inventory.json

    # deterministic checks only, no API key / cost required:
    uv run python -m pipeline.judge.pipeline --dry-run \\
        pipeline/handoff/delivery-2026-09-24/ausserberg/inventory.json
...
from pipeline.judge.adapters import load_categories, load_claims_from_inventory
from pipeline.judge.coverage import compute_coverage, load_reference_categories
from pipeline.judge.injection import deterministic_check
from pipeline.judge.llm import call_judge_ensemble, load_rubric
from pipeline.judge.schemas import (
    BuildJudgeResult,
    Claim,
    CoverageResult,
    InjectionFinding,
