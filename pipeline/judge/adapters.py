...
Currently handles: pipeline/handoff/delivery-2026-09-24/*/inventory.json
...
def _evidence_text_for(service: dict[str, Any], source_refs: list[str]) -> str | None:
    ...
    for every service (see pipeline/handoff/*/documents.jsonl, which is
...
def iter_delivered_inventories(handoff_dir: Path) -> Iterable[Path]:
    """All inventory.json files under a delivery batch, e.g. pipeline/handoff/delivery-2026-09-24/."""
