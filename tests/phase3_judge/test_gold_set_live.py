...
from public_ai_challenge.phase3_judge.pipeline import judge_injection, judge_provenance
from public_ai_challenge.phase3_judge.schemas import Claim

pytestmark = pytest.mark.skipif(
    not os.getenv("OPENAI_API_KEY"), reason="OPENAI_API_KEY not set -- skipping live-model gold set validation"
)

_local_gold = Path(__file__).resolve().parent / "fixtures" / "gold_set.json"
GOLD_SET_PATH = _local_gold if _local_gold.exists() else Path(__file__).resolve().parent.parent / "fixtures" / "gold_set.json"
