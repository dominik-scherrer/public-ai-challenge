...
from public_ai_challenge.alternative_pipeline_publicai.contracts import Discovery, load_discovery
from public_ai_challenge.alternative_pipeline_publicai.runtime import SnapshotRuntime, create_server, date_window


async def test_health_probe_requires_a_responsive_mcp_server() -> None:
    from public_ai_challenge.alternative_pipeline_publicai.runtime import check_health
    ...
    process = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m",
        "public_ai_challenge.alternative_pipeline_publicai.runtime",
        "--discovery",
        str(fixture),
        "--transport",
        "streamable-http",
...
def discovery() -> Discovery:
    return load_discovery(Path(__file__).parents[2] / "src/public_ai_challenge/alternative_pipeline_publicai/fixtures/representative.json")
...
async def test_stdio_transport_negotiates_and_invalid_dates_are_protocol_tool_errors() -> None:
    ...
    parameters = StdioServerParameters(
        ...
        args=["-m", "public_ai_challenge.alternative_pipeline_publicai.runtime", "--discovery", str(fixture)],
    )
