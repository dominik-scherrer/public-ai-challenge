...
from public_ai_challenge.alternative_pipeline_publicai.agents import DiscoveryContext, Inventory, claim_records, create_agents
from public_ai_challenge.alternative_pipeline_publicai.config import Settings
from public_ai_challenge.alternative_pipeline_publicai.contracts import SourceSnapshot
...
async def test_search_is_optional_scoped_and_discovery_only(enabled: bool) -> None:
    ...
    fixture = json.loads(
        (Path(__file__).parents[2] / "src/public_ai_challenge/alternative_pipeline_publicai/fixtures/representative.json").read_text(encoding="utf-8")
    )
...
async def test_form_observations_survive_without_becoming_procedural_requirements() -> None:
    from public_ai_challenge.alternative_pipeline_publicai.crawler import FormField
...
async def test_contact_destinations_remain_citable_after_minimization() -> None:
    from public_ai_challenge.alternative_pipeline_publicai.contracts import Discovery, EvidenceRef, Fact, load_discovery
    from public_ai_challenge.alternative_pipeline_publicai.crawler import extract_html
    from public_ai_challenge.alternative_pipeline_publicai.pipeline import minimize_sources

    discovery = load_discovery(
        Path(__file__).parents[2] / "src/public_ai_challenge/alternative_pipeline_publicai/fixtures/representative.json"
    )
    async with SafeCrawler() as crawler:
