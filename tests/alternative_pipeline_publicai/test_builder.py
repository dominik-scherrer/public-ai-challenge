...
from public_ai_challenge.alternative_pipeline_publicai.builder import BuildError, _run_conformance, build, render_report
...
def test_rebuilding_is_immutable_and_manifest_covers_every_file(tmp_path: Path) -> None:
    ...
    for package in (first, second):
        service = yaml.safe_load((package / "compose.yaml").read_text(encoding="utf-8"))["services"]["municipality"]
        assert "image" not in service  # Compose derives an image from the unique project name.
        assert service["build"] == "."
        assert '"--health-check"' in (package / "Dockerfile").read_text(encoding="utf-8")
    assert original == {
        path.relative_to(first): path.read_bytes() for path in first.rglob("*") if path.is_file()
    }
...
def test_failed_conformance_removes_staging_without_publishing(
    ...
    monkeypatch.setattr("public_ai_challenge.alternative_pipeline_publicai.builder._run_conformance", fail_conformance)
