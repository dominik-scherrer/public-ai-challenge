...
from public_ai_challenge.alternative_pipeline_publicai.agents import Inventory, claim_records, create_agents
from public_ai_challenge.alternative_pipeline_publicai.config import Settings
from public_ai_challenge.alternative_pipeline_publicai.crawler import FetchedPage, Link, SafeCrawler
from public_ai_challenge.alternative_pipeline_publicai.pipeline import DiscoveryError, discover_with_agents

FIXTURE = Path(__file__).parents[2] / "src/public_ai_challenge/alternative_pipeline_publicai/fixtures/representative.json"
...
async def test_complete_pipeline_runs_agents_and_publishes_unique_artifacts(
    ...
    assert json.loads(first.read_text(encoding="utf-8"))["review"]["status"] == "passed"
    assert len(json.loads((first.parent / "review.json").read_text(encoding="utf-8"))["checks"]) > 10
...
async def test_incomplete_semantic_review_never_publishes(
    ...
    diagnostic = json.loads(error.value.diagnostic_path.read_text(encoding="utf-8"))
...
def test_minimization_removes_uncited_text_and_links() -> None:
    from public_ai_challenge.alternative_pipeline_publicai.contracts import load_discovery
    from public_ai_challenge.alternative_pipeline_publicai.pipeline import minimize_sources
...
async def test_provider_error_payload_is_not_exposed(tmp_path: Path) -> None:
    ...
    assert "private-provider-payload" not in error.value.diagnostic_path.read_text(encoding="utf-8")
    assert not list(tmp_path.glob("discovery-*"))
...
def test_minimization_preserves_uninspected_document_evidence() -> None:
    ...
    from public_ai_challenge.alternative_pipeline_publicai.builder import render_report
    from public_ai_challenge.alternative_pipeline_publicai.contracts import Discovery, EvidenceRef, Fact, LinkedDocument, load_discovery
    from public_ai_challenge.alternative_pipeline_publicai.pipeline import minimize_sources
