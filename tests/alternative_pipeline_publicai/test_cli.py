...
def test_cli_reports_safe_discovery_failure_reason(
    ...
    from public_ai_challenge.alternative_pipeline_publicai.cli import _failure
    from public_ai_challenge.alternative_pipeline_publicai.pipeline import DiscoveryError
...
def test_normal_run_creates_discovery_and_tested_package(
    ...
    monkeypatch.setattr("public_ai_challenge.alternative_pipeline_publicai.pipeline.live_agents", model_boundary)
    monkeypatch.setattr("public_ai_challenge.alternative_pipeline_publicai.pipeline.SafeCrawler", OfflineCrawler)
...
def test_cli_model_selection(
    ...
    async def capture_settings(url: str, out: Path, settings: Settings, progress: object) -> Path:
        ...
        return Path(__file__).parents[2] / "src/public_ai_challenge/alternative_pipeline_publicai/fixtures/representative.json"

    monkeypatch.setattr("public_ai_challenge.alternative_pipeline_publicai.pipeline.discover", capture_settings)
    monkeypatch.setattr("public_ai_challenge.alternative_pipeline_publicai.builder.build", lambda path, out: path.parent)
    monkeypatch.setattr("public_ai_challenge.alternative_pipeline_publicai.cli._coverage", lambda path: None)
