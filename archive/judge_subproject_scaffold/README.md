...
An independent subproject, like `pipeline/scout/` and `publicai/`: its own `pyproject.toml` and `uv.lock`, installed and run from inside this directory, not from the repo root. Nothing here depends on Scout's or Factory's code, and nothing there depends on this.
...
Set up and run it standalone, from this directory:

```bash
cd pipeline/judge
uv sync
uv run judge --dry-run ../legacy/handoff/delivery-2026-09-24/ausserberg/inventory.json
```

(`--dry-run` runs only the deterministic checks — no API key, no network call, every model-dependent claim comes back `withheld`.)

Every claim currently comes back **withheld** even without `--dry-run`. That's correct, not broken: `documents.jsonl` is empty in every delivered municipality (see `adapters.py`'s docstring) — the crawler hasn't started capturing source quotes yet, so there is nothing for the provenance judge to check anything against. `tests/test_adapters.py::test_current_seed_data_has_no_evidence_yet` pins this down; when it starts failing, that's the signal that real evidence has landed and this module needs a look, not a bug.
...
uv sync
OPENAI_API_KEY=... uv run pytest tests/test_gold_set_live.py -v
```

## Ensemble: OpenAI + Apertus

`llm.py`'s `JUDGE_MODELS = ("openai", "apertus")` is the intended ensemble — two independent models, because ADR-0004 makes the Judge the *only* gate and one model would be a single point of failure for that. Apertus is called through the exact same env vars (`PUBLIC_AI_ENDPOINT` / `PUBLIC_AI_BASE_URL` / `PUBLIC_AI_API_KEY` / `PUBLIC_AI_MODEL`) that `pipeline/legacy/prototype/model_adapter.py`'s model adapter already uses — one Apertus endpoint configuration serves both. The root `.env.example` doesn't template these yet (only `OPENAI_API_KEY`); add them to your own `.env` by hand until it does.
...
```
pipeline/judge/
  pyproject.toml    own subproject — uv sync/run here, not from the repo root
  uv.lock
  src/judge/
    ...
    pipeline.py     orchestrates all three checks, CLI entry point (`uv run judge`)
