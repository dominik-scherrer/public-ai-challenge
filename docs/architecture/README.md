...
The team's `pipeline/` workstream (Scout ingestion + handoff) moved fast while these ADRs sat unreferenced in a personal folder. None of them have been formally contradicted, but several are not yet implemented, and one is actively at risk. Re-check this table as `pipeline/` matures — don't treat it as permanent.
...
| [0004](adr/0004-zero-maintenance-at-the-municipality.md) | Zero maintenance at the municipality; the Judge is the only quality gate | **Gap.** `pipeline/EVALS.md` and `pipeline/PROVENANCE_AND_TRUST.md` cover metrics (recall, precision, provenance coverage) and evidence classification, but no component decides go/no-go per attribute or per Build yet. Nobody currently owns building the Judge. |
...
| [0007](adr/0007-service-inventory-is-typed-data-with-injection-check.md) | Service Inventory is typed data, with a separate injection check | Compatible and substantially extended by `pipeline/PROVENANCE_AND_TRUST.md`'s evidence/classification model — but the explicit injection check (foreign-domain links, payment details, instructions-to-a-model) isn't named anywhere in the new docs. Worth confirming it's still planned. |

## On Patrick's plan and the delivered data

`ideas-patrick/hackathon-mvp-plan.md` and the fixed-7-tool "Factory" design driven from it are **not** treated as authoritative for this architecture. The municipality inventories already delivered under `pipeline/handoff/delivery-2026-09-24/` (Ausserberg, Binn, Bosco/Gurin, Ilanz/Glion, Biel/Bienne, Lausanne, Lugano, Zürich) are kept regardless, as reference data/examples of the target Service Inventory shape — independent of which plan produced them.
